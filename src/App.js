...
class App extends Component {
  ...
  handleRemove = (id, email) => {
    alert(`A person with a email ${email} was removed from your network`);
    const user = this.state.network;
    user.splice(id, 1);
    this.setState({ network: user });
  };
      ...
      <div className="App">
        ...
        <div className="list">
          {this.state.network.map((p, i) => (
            <Person key={i} person={p} index={i} remove={this.handleRemove} />
          ))}
        </div>
      </div>
    );
